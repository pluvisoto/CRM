...
import { Edit2, Trash2, CheckCircle, FileText, Receipt, CreditCard, Calendar, ArrowUpCircle, Check, AlertCircle, Plus } from 'lucide-react';
import AccountForm from './AccountForm';

const AccountsPayable = ({ initialFilter = 'all', onUpdate, selectedMonth, onMonthChange, monthlyStats }) => {
    const { user } = useAuth();
    ...
    const markAsPaid = async (id) => {
        try {
            const { error } = await supabase
                .from('accounts_payable')
                .update({ paid_date: new Date().toISOString().split('T')[0], status: 'paid' })
                .eq('id', id);
    ...
    const getStatusBadge = (status) => {
        const styles = {
            ...
            paid: { bg: 'rgba(239, 68, 68, 0.1)', color: '#f87171', label: 'Paga' }
        };
    ...
    return (
        <div className="accounts-container">
            <div style={{ display: 'flex', gap: '2rem', marginBottom: '1rem', width: '100%' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', gap: '1rem' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                        ...
                        {/* ACTION BUTTON */}
                        <button
                            className="action-btn-primary payable"
                        ...
                        <div key={date} className="timeline-group">
                            ...
                            <div className="timeline-items">
                                {groupedAccounts[date].map(account => {
                                    ...
                                    const isOverdue = !isPaid && new Date(account.due_date) < new Date().setHours(0, 0, 0, 0);

                                    return (
                                        <div
                                            key={account.id}
                                            className={`timeline-item payable ${isPaid ? 'complete' : ''}`}
                                        >
                                            <div className="item-checkbox">
                                                <button
                                                    className={`check-btn ${isPaid ? 'checked' : ''}`}
                                                    onClick={() => markAsPaid(account.id)}
                                                    disabled={isPaid}
                                                    title={isPaid ? "Paga" : "Marcar como paga"}
                                                >
                                            ...
                                            <div className="item-content">
                                                <div className="item-header">
                                                    <span className="item-type payable">
                                                        <ArrowUpCircle size={12} />
                                                        Pagar
                                                    </span>
                                                    <span className="item-description">
                                                        {account.description}
                                                        {isOverdue && !isPaid && <span className="status-badge overdue">Vencida</span>}
                                                    </span>
                                                </div>
            ...
            <style>{`
                ...
                .timeline-item.payable::before {
                    background: linear-gradient(to bottom, #ef4444, #dc2626);
                    box-shadow: 0 0 10px rgba(239, 68, 68, 0.5);
                }

                .timeline-item.payable {
                    background: linear-gradient(135deg, rgba(239, 68, 68, 0.03) 0%, rgba(30, 30, 40, 0.4) 100%);
                }
